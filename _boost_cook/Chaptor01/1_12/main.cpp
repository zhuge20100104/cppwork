...
void sample5() {
    ...
    std::string s("Expense copy constructor is NOT called when binding");
    std::size_t count0 = std::count_if(v.begin(), v.end(), 
        [&s](const std::string& x) {
            return x < s;
        });

    std::size_t count1 = std::count_if(v.begin(), v.end(), 
        boost::bind(
            std::less<std::string>(),
            _1,
            boost::cref(s)
