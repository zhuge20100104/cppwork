...
#define S_FUNCTION_NAME first_1_msf
#endif
...
slmrInitializeIOPortDataVectors ( S , 5 , 2 ) ; if ( ! ssSetNumInputPorts ( S
...
ssSetNumOutputPorts ( S , 2 ) ) return ; if ( !
...
USE_CUSTOM_SIM_STATE ) ; mr_first_1_RegisterSimStateChecksum ( S ) ;
...
SS_SIMMODE_SIZES_CALL_ONLY ) { int_T retVal = 1 ; mr_first_1_MdlInfoRegFcn (
S , "first_1" , & retVal ) ; if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( gkrrblwlytk ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { gkrrblwlytk * dw = (
gkrrblwlytk * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } ju41pvzfyv
...
* S , int_T tid ) { gkrrblwlytk * dw = ( gkrrblwlytk * ) ssGetDWork ( S , 0 )
; real_T const * i_manmbq2xhl = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ;
real_T const * i_hsvdwemsbq = ( real_T * ) ssGetInputPortSignal ( S , 1 ) ;
real_T const * i_ncufa20w3e = ( real_T * ) ssGetInputPortSignal ( S , 2 ) ;
real_T const * i_c4hp3vkxfi = ( real_T * ) ssGetInputPortSignal ( S , 3 ) ;
real_T const * i_notaroanaw = ( real_T * ) ssGetInputPortSignal ( S , 4 ) ;
real_T * o_B_1_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T *
o_B_1_2 = ( real_T * ) ssGetOutputPortSignal ( S , 1 ) ; if ( tid !=
CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { first_1 ( i_manmbq2xhl ,
i_hsvdwemsbq , i_ncufa20w3e , i_c4hp3vkxfi , i_notaroanaw , o_B_1_1 , o_B_1_2
) ; } } static void mdlTerminate ( SimStruct * S ) { gkrrblwlytk * dw = (
gkrrblwlytk * ) ssGetDWork ( S , 0 ) ; grrifbxgfo ( & ( dw -> rtm ) ) ;
return ; }
...
mdlrefDW = mr_first_1_GetDWork ( ssGetDWork ( S , 0 ) ) ; mxSetFieldByNumber
( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_first_1_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss , 0 , 2
...
mr_first_1_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss , 0 , 1
