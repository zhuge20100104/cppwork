...
TEST(TfArrayOpsTests, WholeFileReader) {
    // 读取整个文件作为string
    // https://www.tensorflow.org/api_docs/cc/class/tensorflow/ops/whole-file-reader
    ...
    auto reader = ops::WholeFileReader(root);
    auto read_res = ops::ReaderRead(root.WithOpName("whole_read"), reader, queue_);
    ...
    std::array<std::string, 2> arr {"abcdefghijkl", "ABCDEFGHIJKL"};
