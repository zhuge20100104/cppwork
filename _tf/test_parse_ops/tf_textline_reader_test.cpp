...
TEST(TfArrayOpsTests, FixLenRecordReader) {
    ...
    auto tensor_ = ops::Const(root, {"/cppwork/_tf/test_parse_ops/data/3.txt"});
    ...
    auto reader = ops::TextLineReader(root);
    auto read_res = ops::ReaderRead(root.WithOpName("line_read"), reader, queue_);
    ...
    std::array<std::string, 6> arr {"abcd", "efgh", "ijkl", "lmno", "pqrs","tuvw"};
